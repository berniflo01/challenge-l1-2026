...
async function chargerJournee(n) {
  // Compteur pour ignorer les reponses obsoletes : en cliquant vite sur
  // precedent/suivant, 2 requetes partent en parallele et la plus lente
  // peut ecraser l'affichage de la plus recente (titre journee 1, matchs
  // de la journee 2). On ne garde que la reponse du dernier clic.
...
function construireLigneResultat(m) {
  ...
  // Meme code couleur que sur l'ecran des matchs a venir : vert si des
  // points ont ete gagnes, rouge si rate, gris si aucun prono saisi.
  if (m.statut === 'termine') {
...
function construireBlocPronosFinalAutres_() {
  ...
  btn.addEventListener('click', async () => {
    ...
    // Une carte par joueur plutot qu'un tableau a 7 colonnes : sur mobile
    // (~380px), 6 colonnes d'equipes debordent et les 3 dernieres sont
    // coupees. Ici chaque joueur tient dans sa carte, sans defilement.
