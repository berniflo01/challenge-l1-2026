...
async function chargerPronoFinal() {
  ...
  const optionsTop3 = '<option value="">–</option>' + listeEquipesGlobale
    .map(e => `<option value="${e.nom}">${e.nom}${e.coteTop3 ? ` (${e.coteTop3})` : ''}</option>`).join('');
  const optionsDescente = '<option value="">–</option>' + listeEquipesGlobale
    .map(e => `<option value="${e.nom}">${e.nom}${e.coteDescente ? ` (${e.coteDescente})` : ''}</option>`).join('');
