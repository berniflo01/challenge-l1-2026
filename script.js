...
async function apiGet(action, params = {}) {
  ...
  try {
    ...
    // Reseau coupe, quota Apps Script depasse, ou reponse non-JSON (page
    // d'erreur renvoyee par le serveur) : avant ce fix, ca faisait planter
    // silencieusement tout le script en cours (souvent init()), avec
    // l'ecran qui reste noir puisque rien n'a jamais ete affiche par-dessus
    // le fond sombre. Desormais on renvoie toujours un objet exploitable.
    return { success: false, reason: 'erreur_reseau' };
  }
}
...
async function init() {
  ...
  document.getElementById('btn-reessayer-chargement').addEventListener('click', () => location.reload());

  let reponseMoi;
  try {
    [, reponseMoi] = await Promise.all([promesseJoueurs, promesseMoi]);
  } catch (e) {
    // Dernier filet de securite : meme si apiGet/apiPost ne devraient plus
    // jamais lever d'exception (voir plus haut), on prefere un message
    // clair a un ecran noir muet si jamais un cas non prevu survient.
    afficherErreurChargement_();
    return;
  }
  ...
  if (reponseMoi && reponseMoi.reason === 'erreur_reseau') {
    // Ne PAS effacer le token ici : ce n'est pas un token invalide, juste
    // un souci reseau/serveur passager. L'effacer forcerait une reconnexion
    // pour rien une fois le probleme resolu.
    afficherErreurChargement_();
    return;
  }
  if (token) clearToken();
...
function afficherErreurChargement_() {
  document.getElementById('vue-erreur-chargement').style.display = 'flex';
}
...
async function chargerListeJoueurs() {
  ...
  if (reponse.success) {
    ...
    select.innerHTML = '<option value="">Erreur de chargement, recharge la page</option>';
  }
}
...
async function chargerJournee(n) {
  const monChargement = ++chargementEnCours;
  journeeCourante = n;
  await peuplerSelectJournee_();
  if (monChargement !== chargementEnCours) return;

  document.getElementById('select-journee').value = n;
  majFlechesNavigation_();
  chargerBandeauBonus(n);
  ...
  // Si des matchs sont en cours, va chercher la minute de jeu en plus
  // (endpoint dedie, pas stocke en Sheet) et l'attache a chaque match
  // avant de construire les lignes.
  if (reponse.matchs.some(m => m.statut === 'en_cours')) {
    const reponseMinutes = await apiGet('minutesEnDirect', { token: getToken(), journee: n });
    if (monChargement !== chargementEnCours) return;
    if (reponseMinutes.success) {
      reponse.matchs.forEach(m => {
        m.minuteAffichee = reponseMinutes.minutes[`${m.domicile}|${m.exterieur}`] || '';
      });
    }
  }
...
async function chargerBandeauBonus(journee) {
  const conteneur = document.getElementById('bandeau-bonus');
  const reponse = await apiGet('bonusJournee', { token: getToken(), journee });
  if (!reponse.success || !reponse.paliers.length) {
    conteneur.style.display = 'none';
    return;
  }

  conteneur.className = 'bandeau-bonus';
  conteneur.style.display = 'block';
  conteneur.innerHTML = '';

  const btnTitre = document.createElement('button');
  btnTitre.className = 'bandeau-bonus-titre';
  btnTitre.textContent = 'Bonus de la journée ▾';

  const contenu = document.createElement('div');
  contenu.className = 'bandeau-bonus-contenu';
  contenu.style.display = 'none';

  const detail = document.createElement('div');
  detail.className = 'detail-bonus';
  detail.style.display = 'none';

  reponse.paliers.forEach((p, index) => {
    const ligne = document.createElement('div');
    ligne.className = 'palier-bonus';

    const tag = document.createElement('span');
    tag.className = 'tag-bonus' + (index === 0 ? ' meilleur' : '');
    tag.textContent = `${p.nbBonnes}/9 +${p.bonus}`;
    ligne.appendChild(tag);

    const noms = document.createElement('span');
    noms.className = 'noms-bonus';
    p.joueurs.forEach((j, i) => {
      const lien = document.createElement('button');
      lien.className = 'lien-joueur-bonus';
      lien.textContent = j.nom;
      lien.addEventListener('click', () => afficherDetailBonus_(detail, j, journee));
      noms.appendChild(lien);
      if (i < p.joueurs.length - 1) noms.appendChild(document.createTextNode(', '));
    });
    ligne.appendChild(noms);
  ...
  btnTitre.addEventListener('click', () => {
    const ouvert = contenu.style.display !== 'none';
    contenu.style.display = ouvert ? 'none' : 'block';
    btnTitre.textContent = ouvert ? 'Bonus de la journée ▾' : 'Bonus de la journée ▴';
  });

  conteneur.appendChild(btnTitre);
  conteneur.appendChild(contenu);
}

async function afficherDetailBonus_(detail, joueur, journee) {
  const dejaOuvertPour = detail.dataset.idJoueur;
  if (dejaOuvertPour === String(joueur.idJoueur) && detail.style.display !== 'none') {
    detail.style.display = 'none';
    return;
  }
  detail.dataset.idJoueur = joueur.idJoueur;
  detail.style.display = 'block';
  detail.innerHTML = '<p class="note">Chargement...</p>';

  const reponse = await apiGet('pronosJoueurJournee', {
    token: getToken(), idJoueurCible: joueur.idJoueur, journee,
  });
  if (!reponse.success) {
    detail.innerHTML = '<p class="note">Erreur de chargement.</p>';
    return;
  }

  const lignes = reponse.matchs.map(m => {
    const libelle = `${nomCourt_(m.domicile)} - ${nomCourt_(m.exterieur)}`;
    if (!m.termine) {
      return `<div class="ligne-detail-bonus a-venir"><span>${libelle}</span><span>à venir</span></div>`;
    }
    const resultat = m.points > 0
      ? `<span class="detail-gagne">${m.prono || '–'} · +${formaterPoints_(m.points)}</span>`
      : `<span class="detail-perdu">${m.prono || '–'} · raté</span>`;
    return `<div class="ligne-detail-bonus"><span>${libelle}</span>${resultat}</div>`;
  }).join('');

  detail.innerHTML = `<div class="detail-bonus-titre">Pronos de ${joueur.nom}</div>${lignes}`;
}
...
function construireLigneResultat(m) {
  ...
  spanScore.textContent = m.statut !== 'termine'
    ? 'À venir'
    : (scoreConnu ? `${m.scoreDomicileReel} - ${m.scoreExterieurReel}` : (m.resultat || '–'));
...
function construireLigneMatch(m) {
  ...
  if (m.statut === 'termine') {
    ...
    const base = scoreConnu ? `${m.scoreDomicileReel} - ${m.scoreExterieurReel}` : '● LIVE';
    spanVs.innerHTML = m.minuteAffichee
      ? `<span>${base}</span><span class="minute-live">${m.minuteAffichee}</span>`
      : `<span>${base}</span>`;
...
function construireBlocPronosFinalAutres_() {
  ...
  btn.addEventListener('click', async () => {
    ...
    tableau.innerHTML = reponse.joueurs.map(j => {
      const cases = [
        ['1er', j.equipe1], ['2ème', j.equipe2], ['3ème', j.equipe3],
        ['16ème (L2)', j.equipe16], ['17ème', j.equipe17], ['18ème', j.equipe18],
      ];
      const bloc = (debut, fin) => cases.slice(debut, fin).map(([label, equipe]) => `
        <div class="case-final-joueur">
          <div class="case-final-label">${label}</div>
          <div class="case-final-equipe">${equipe ? nomCourt_(equipe) : '—'}</div>
        </div>
      `).join('');
      return `
        <div class="carte-final-joueur${j.aProno ? '' : ' sans-prono'}">
          <div class="carte-final-nom">${j.nom}</div>
          <div class="carte-final-ligne">${bloc(0, 3)}</div>
          <div class="carte-final-ligne">${bloc(3, 6)}</div>
        </div>
      `;
    }).join('');
  });
...
async function chargerClassement() {
  ...
  entetes.push('<th class="col-joueur">Joueur</th><th class="col-chiffre">Points</th><th class="col-chiffre">Gain</th>');
  ...
  reponse.classement.forEach(c => {
    ...
    else if (!c.gain) tr.className = 'hors-itm';
    ...
    cellules += `<td>${c.rang}</td>`
      + `<td class="col-joueur">${c.prenom} ${c.nom} ${badgesJoueur_(c.nom, c.prenom)}</td>`
      + `<td class="col-chiffre">${formaterPoints_(c.points)}</td>`
      + `<td class="col-chiffre col-gain">${c.gain ? c.gain + '€' : '—'}</td>`;
    if (config.champ) {
...
function initEcranAdmin() {
  ...
  const selectCotes = document.getElementById('admin-journee-cotes');
  for (let n = 1; n <= TOTAL_JOURNEES; n++) {
    const opt = document.createElement('option');
    opt.value = n;
    opt.textContent = `Journée ${n}`;
    selectCotes.appendChild(opt);
  }
  selectCotes.value = journeeCourante;

  document.getElementById('btn-admin-cotes').addEventListener('click', async () => {
    const statut = document.getElementById('statut-admin-cotes');
    statut.textContent = 'Récupération en cours...';
    const journee = Number(selectCotes.value);
    const reponse = await apiPost('recupererCotesAdmin', { journee });
    if (!reponse.success) {
      statut.textContent = reponse.reason === 'sofascore_indisponible'
        ? 'SofaScore ne répond pas, réessaie plus tard.'
        : 'Erreur, réessaie.';
      return;
    }
    let texte = `${reponse.corriges} match(s) mis à jour.`;
    if (reponse.echecs.length) texte += ` Échecs : ${reponse.echecs.join(' · ')}`;
